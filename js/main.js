/*  
Set up the main.js page for the A* Search
*/
/* global Graph, astar, $ */
// create walls and grid
var WALL = 0,
...
GraphSearch.prototype.initialize = function() {
    ...
    var cellWidth = ($graph.width()/this.opts.gridSize)-0,  // -2 for border
        cellHeight = ($graph.height()/this.opts.gridSize)-0,
...
//-------------- Click randomly----------------
     /**
     * Looping select positions randomly
     */
    //   setInterval(function() {
    //      var x = $('#cell_'+ getRandom()+'_'+ getRandom());
    //      console.log(x);
    //      self.cellClicked(x);
    //   }, 6000);
    // };
...
// *
//  * To get random number betwwen 0 - 40
//  * @return {int}
...
// get deta to $end  after the click cell
...
/// changed started snake animetion from here----------
GraphSearch.prototype.animatePath = function(path) {

    var grid = this.grid,
        timeout = 1000 / grid.length,
        elementFromNode = function(node) {
          // console.log('elementFromNode = function(node)=>', node);
    ...
    var addClass = function(path, i) {
        if(i >= path.length && !self.startedToRemove) { // Finished showing path, now remove
          // console.log('Finished showing path, now remove');
            return removeClass(path, 0);
        }

        if(i >=10 && !self.startedToRemove){
          self.startedToRemove = true;
          removeClass(path, 0);
        }
